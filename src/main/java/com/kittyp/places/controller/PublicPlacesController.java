...
import com.kittyp.places.service.PlacesRateLimiter;
...
public class PublicPlacesController {
	...
	@PostMapping(ApiUrl.PUBLIC_PLACES_AUTOCOMPLETE)
	public ResponseEntity<SuccessResponse<AutocompleteResponse>> autocomplete(
			@RequestBody AutocompleteRequest request, HttpServletRequest http) {
		placesRateLimiter.checkPublic(clientKey(http));
	...
	@PostMapping(ApiUrl.PUBLIC_PLACES_DETAILS)
